    ...
    def fit(self):
        ...
        if os.path.exists(os.path.join(self.path_baofit, "fit_results.txt")) and not self.overwrite:
            ...
            if chi2_vector[0] > chi2_best + 1 and chi2_vector[-1] > chi2_best + 1:
                ...
                if nbins_eff > 1:
                    # Plot the bin-average w(theta) (can be useful for visualization, but it's not physically meaningful. 
                    # Also, it's not very useful if it averages a wide redshift range since the BAO peak evolves). It fails
                    # if there are no common scales between different redshift bins, so I'll leave it with a try instance
                    try:
                        ...
                        print("Not able to produce the bin-averaged w(theta) plot!")
